...
const createCustomIcon = (colorName) => {
  return new L.Icon({
...
const getMarkerIcon = (dust) => {
  if (dust === null || dust === undefined) {
    return createCustomIcon("grey");
  }
  
  if (dust <= 25.0) {
    return createCustomIcon("green");
  } else if (dust <= 75.0) {
    return createCustomIcon("gold");
  } else {
    return createCustomIcon("red");
  }
};

function Map({ position, dustValue, tempValue, humidValue, title }) {
    const currentIcon = getMarkerIcon(dustValue);
